...
function drawGallery(picturesToShow, works) {
    ...
    let buttonBackwards = navigation.find(
        ".works-gallery__nav-button--backwards"
    );
    let buttonForwards = navigation.find(".works-gallery__nav-button--forwards");
    let yyy = 0;
    let zzz = picturesToShow;
    for (let i = yyy; i < zzz; i++) {
        $(works.children()[i]).addClass("works-gallery__preview--show");
    }
    buttonBackwards.on("click", function() {
        let showingPictures = works.find(".works-gallery__preview--show");
        for (let i = 0; i <= showingPictures.length; i++) {
            $(showingPictures[i]).removeClass("works-gallery__preview--show");
        }
        yyy -= picturesToShow;
        zzz -= picturesToShow;
        for (let i = yyy; i < zzz; i++) {
